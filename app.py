...
@app.route('/hunt-import')
def hunt_import_page():
    """Hunt Import — triage CTI-ATTACKCollector JSON reports."""
    return render_template('hunt_import.html')
...
@csrf.exempt
@app.route('/api/hunt/import', methods=['POST'])
@limiter.limit("10 per minute")
def import_hunt_report():
    """
    Accepts a CTI-ATTACKCollector JSON report, validates the schema,
    sanitises all fields server-side, and returns a clean payload
    to the triage UI.  No data is stored or logged beyond the
    actor name and technique count in the application log.
    """
    if not validate_json_request():
        return jsonify({"error": "Invalid request. JSON required."}), 400
    ...
    # ── Schema validation ─────────────────────────────────────────────────────
    if 'report_meta' not in data or 'techniques' not in data:
        return jsonify({
            "error": (
                "Unrecognised format. Expected a CTI-ATTACKCollector JSON report "
                "with 'report_meta' and 'techniques' keys."
            )
        }), 400
    ...
    if not isinstance(meta, dict):
        return jsonify({"error": "'report_meta' must be an object."}), 400
    if not isinstance(techniques, list):
        return jsonify({"error": "'techniques' must be an array."}), 400
    if len(techniques) == 0:
        return jsonify({"error": "No techniques found in report."}), 400
    if len(techniques) > 500:
        return jsonify({"error": "Report exceeds maximum of 500 techniques."}), 400

    # ── Sanitise meta ─────────────────────────────────────────────────────────
    safe_meta = {
        "actor_name":    sanitize_input(str(meta.get("actor_name",    "")), 200),
        "actor_aliases": sanitize_input(str(meta.get("actor_aliases", "")), 500),
        "generated_at":  sanitize_input(str(meta.get("generated_at",  "")),  50),
        "source":        sanitize_input(str(meta.get("source",        "")), 200),
    }
    if not safe_meta["actor_name"]:
        return jsonify({"error": "report_meta.actor_name is required."}), 400
    ...
    for t in techniques:
        if not isinstance(t, dict):
            continue
        raw_id = sanitize_input(str(t.get("technique_id", "")), 20)
        # Reject entries that don't look like ATT&CK IDs (T#### or T####.###)
        if not raw_id.startswith("T") or len(raw_id) < 5:
            continue
        safe_techniques.append({
            "technique_id":    raw_id,
            "technique_name":  sanitize_input(str(t.get("technique_name",  "")), 200),
            "kill_chain":      sanitize_input(str(t.get("kill_chain",      "")), 200),
            "platforms":       sanitize_input(str(t.get("platforms",       "")), 300),
            "data_sources":    sanitize_input(str(t.get("data_sources",    "")), 1000),
            "detection":       sanitize_input(str(t.get("detection",       "")), 3000),
            "hunt_hypothesis": sanitize_input(str(t.get("hunt_hypothesis", "")), 1000),
            "attack_url":      sanitize_input(str(t.get("attack_url",      "")), 200),
            "is_subtechnique": bool(t.get("is_subtechnique", False)),
        })

    if not safe_techniques:
        return jsonify({"error": "No valid ATT&CK techniques found after validation."}), 400

    safe_meta["technique_count"] = len(safe_techniques)

    logger.info(
        f"Hunt import: actor={safe_meta['actor_name']!r} "
        f"techniques={safe_meta['technique_count']}"
    )

    return jsonify({
        "success":    True,
        "meta":       safe_meta,
        "techniques": safe_techniques,
    })
